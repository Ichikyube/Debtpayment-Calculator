<div id="hilanginscroll" x-data="{modelOpen: false}" x-init="[listData()]" x-cloak>
    ...
    <x-loading />
    <!-- notification -->
    <button x-show="!loading && messages" x-init="timeNotif" type="button" x-on:click="closeNotif" id="alert-4" class="absolute top-0 right-0 z-50 flex items-center h-6 px-4 space-x-2 text-white transition bg-green-500 rounded-md justify-self-end w-fit hover:bg-green-600"
        ...
        <template x-for="data in list">
            <div  x-data="{ open: false, delete1: false}" class="bg-[#F7D3C2] py-[24px] lg:px-[45px] px-5 rounded-[15px] mb-5 shadow-sm hover:shadow-md transition-shadow duration-300 ease-in-out">
                ...
                <div class="flex flex-wrap justify-between break-words lg:flex-nowrap">
                    <div class="flex flex-col flex-wrap w-full mt-2 border-b-2 border-dashed lg:border-b-0 sm:flex-row md:flex-row lg:flex-row justify-evenly">
                        <div>
